...
fertcal4 <- subset(nfhs4, select = c("caseid", "v017", "v018", "v019", "vcal_7", "trim_cal7", "rev_cal7"))
...
nfhs4_preg <- nfhs4_preg %>% select(c(caseid, v017, v018, v019, vcal_7, prev_stillbirth, vcal_len, lt, lt_cmc, la, la_cmc, lm, lm_cmc, ls, ls_cmc, lb, lb_cmc, true_stillbirth))
...
nfhs4_preg <- nfhs4_preg %>% select(c(caseid, prev_stillbirth, outcome, year_event1, month_event1, interval_preg))
...
nfhs5_preg <- subset(nfhs5, select = c("caseid", "v017", "v018", "v019","scal_1"))
...
#keeping only variables needed for merge
nfhs5_preg <- nfhs5_preg %>% select(c(caseid, prev_stillbirth, outcome, year_event1, month_event1, interval_preg))
...
dlhs_preg <- dlhs_preg %>% select(c(id_person, outcome1, outcome2, outcome3, outcome4, outcome5, outcome6, month_event1,
...
dlhs_preg <- dlhs_preg %>% mutate(prev_stillbirth = case_when(outcome2 > 1 ~ 1,
                                                              outcome3 > 1 ~ 1,
                                                              outcome4 > 1 ~ 1,
                                                              outcome5 > 1 ~ 1,
                                                              outcome6 > 1 ~ 1,
...
dlhs_preg <- dlhs_preg %>% select(c(caseid, outcome, interval_preg, prev_stillbirth, survey, date_outcome))
...
dlhs_nfhs_preg <- read.csv("dlhs_nfhs_preg-Caroline’s Mac mini.csv")
