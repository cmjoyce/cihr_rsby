...
hh <- read_dta("IAHR74FL.DTA", col_select = c(1:200))
#which(colnames(hh) == "sh58") ## keeping only case IDs and BPL card for HH

#keeping only variables needed for asset index + merge -- cluster number, household number, state
hh <- hh %>% select(hv001, hv002, hv024, hv225, hv237, hv237a,
                    hv237b, hv237c, hv237d, hv237f,
                    hv237g, hv237h, hv237x, hv237z, hv243a, hv243c, hv244,
                    sh37o, sh58)

#renaming for merging so variables match between datasets (except bpl variable)
hh <- rename(hh, v001 = hv001, v002 = hv002, v024 = hv024, toilet_shared = hv225, 
             water_treat = hv237, boil_water = hv237a, bleach_water = hv237b, strain_water = hv237c,
             filter_water = hv237d, settle_water = hv237f, alum_water = hv237g, electr_purif_water = hv237h,
             other_water = hv237x, dk_water = hv237z, mobile_phone = hv243a, 
             animal_cart = hv243c, own_agric_land = hv244, computer = sh37o, bpl = sh58)
...
nfhs4 <- rename(nfhs4, state = v024, wt = v005,  district = sdistri, psu = v021, religion = v130, caste = v131,
                rural_urban = v025, caste_group = s116, source_water = v113,  
                toilet = v116, radio = v120, tv = v121, fridge = v122, bike = v123, 
                motorcycle = v124, car = v125, 
                other_phone = v153, cook_fuel = v161, 
...
nfhs4 <- nfhs4 %>% select(caseid, wt, month, year, date, age, v018, v019, 
                          ...
                          toilet_shared, radio, tv, fridge, bike,
                          motorcycle, car,
                          other_phone, cook_fuel,
                          water_treat, boil_water, bleach_water, strain_water, filter_water,
                          settle_water, alum_water, electr_purif_water, other_water, dk_water,
                          mobile_phone, animal_cart, own_agric_land, computer,
                          ...
                          type_delivery, place_delivery, conducted_delivery, pp_checkup, health_ins, esis, cghs, shis, rsby, chip,
                          other_employer, 
...
#doing this after merge

#adding in NFHS4 district id variable from seperate district harmonization file. 
# Using nfhs4_district_names file.

nfhs4_district_names <- nfhs4_district_names %>% filter(!is.na(nfhs4_district_names$NFHS4_id))

nfhs4 <- left_join(nfhs4, nfhs4_district_names, 
                   by = c("district" = "NFHS4_id"))


#this introduced multiple matches to district. Doing distinct to remove the multiples.
nfhs4 <- distinct(nfhs4)

#checking
#nfhs4_try <- nfhs4_try %>% select(c(caseid, state, district, dist_id))
...
hh_nfhs5 <- read_dta("IAHR7AFL.DTA", col_select = c(1:30, 73:116, 150:260))
#which(colnames(hh) == "sh58") ## keeping only case IDs, asset index questions, and and BPL card for HH

#keeping only variables needed for merge -- cluster number, household number, state, and BPL card
hh_nfhs5 <- hh_nfhs5 %>% select(hv001, hv002, hv024, hv225, hv237, hv237a, hv237b, hv237c, hv237d, hv237f, hv237g, hv237h,
                                hv237x, hv237z, hv243c, hv243e, hv244, sh75)

#renaming for merging so variables match between datasets (except bpl variable)
hh5 <- dplyr::rename(hh_nfhs5, v001 = hv001, v002 = hv002, v024 = hv024, toilet_shared = hv225, water_treat = hv237,
                     boil_water = hv237a, bleach_water = hv237b, strain_water = hv237c, filter_water = hv237d, settle_water = hv237f,
                     alum_water = hv237g, electr_purif_water = hv237h, other_water = hv237x, dk_water = hv237z, animal_cart = hv243c, 
                     computer = hv243e, own_agric_land = hv244,
                     bpl= sh75)
...
nfhs5 <- rename(nfhs5, state = v024, wt = v005,  district = sdist, psu = v021, religion = v130, caste = v131,
                rural_urban = v025, caste_group = s116, source_water = v113,  
                toilet = v116, radio = v120, tv = v121, fridge = v122, bike = v123, motorcycle = v124, car = v125, 
                other_phone = v153, cook_fuel = v161, mobile_phone = v169a,
                health_ins = v481, esis = v481a, cghs = v481b, shis = v481c,
                rsby = v481d, chip = v481e, other_employer = v481f, reimburse = v481g,
                other_private = v481h, other_insurance = v481x, 
                date = v008, month = v006, year = v007,
...
nfhs5 <- nfhs5 %>% select(caseid, wt, month, year, date, age, v018, v019, 
                          state, psu, rural_urban, years_school, source_water, toilet,
                          toilet, radio, tv, fridge, bike, motorcycle, car,
                          other_phone, cook_fuel, mobile_phone, toilet_shared, water_treat,
                          boil_water, bleach_water, strain_water, filter_water, settle_water, alum_water, 
                          electr_purif_water, other_water, dk_water, animal_cart, computer, own_agric_land,
                          religion, caste, v150, v190, v191,
                          tot_live_births, v208, v209, age_first_birth, 
                          current_method, fp_future, v367, index_mob, index_yob, month_received_anc, num_anc_visits,  
...
#adding in NFHS5 district id variable from seperate district harmonization file. 
# Using nfhs4_district_names file.

nfhs5_district_names <- nfhs5_district_names %>% filter(!is.na(nfhs5_district_names$NFHS5_id))

nfhs5 <- left_join(nfhs5, nfhs5_district_names, 
                   by = c("district" = "NFHS5_id"))


#this introduced multiple matches to district. Doing distinct to remove the multiples.
nfhs5 <- distinct(nfhs5)

#checking
#nfhs5_try <- nfhs5_try %>% select(c(caseid, state, district, dist_id))
...
#making nfhs state_district variable to match dlhs. We will not use this. Will use dlhs state_dist for district harmonization as each value is not unique.
nfhs$state_dist <- paste(nfhs$state, nfhs$district, sep ="")

nfhs <- nfhs %>% relocate(state_dist, .after = wt)
...
dlhs$district <- as.numeric(dlhs$district)
nfhs$state_dist <- as.numeric(nfhs$state_dist)

library(janitor)
compare_df_cols(dlhs, nfhs)
...
dlhsnfhs_preg_mothercov <- dlhsnfhspreg
